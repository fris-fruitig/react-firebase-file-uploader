  ...
  _createClass(FirebaseFileUploader, [{
    ...
    value: function startUpload(file) {
      ...
      var _props = this.props,
          _props$beforeUploadSt = _props.beforeUploadStart,
          beforeUploadStart = _props$beforeUploadSt === undefined ? function (file) {
        return Promise.resolve(file);
      } : _props$beforeUploadSt,
      ...
      beforeUploadStart(file).then(function (preprocessedFile) {
        var task = storageRef.child(filenameToUse).put(preprocessedFile, metadata);

        if (onUploadStart) {
          onUploadStart(preprocessedFile, task);
        }

        var handleProgress = onProgress && function (snapshot) {
          return onProgress(Math.round(100 * snapshot.bytesTransferred / snapshot.totalBytes), task);
        };
        var handleError = onUploadError && function (error) {
          return onUploadError(error, task);
        };
    ...
    value: function render() {
      var _props2 = this.props,
          ...
          beforeUploadStart = _props2.beforeUploadStart,
          ...
          props = _objectWithoutProperties(_props2, ['storageRef', 'beforeUploadStart', 'onUploadStart', 'onProgress', 'onUploadSuccess', 'onUploadError', 'randomizeFilename', 'metadata', 'filename', 'hidden', 'as']);
