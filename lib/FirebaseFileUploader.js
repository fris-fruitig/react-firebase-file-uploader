...
function generateRandomFilename(currentFilename) {
  var extension = /(?:\.([^.]+))?$/.exec(currentFilename)[0];
  return (0, _uuid.v4)() + extension;
}
...
var FirebaseFileUploader = function (_Component) {
  ...
  _createClass(FirebaseFileUploader, [{
    ...
    value: function startUpload(file) {
      ...
      var currentFilename = filename || file.name;
      var filenameToUse = randomizeFilename ? generateRandomFilename(currentFilename) : currentFilename;
