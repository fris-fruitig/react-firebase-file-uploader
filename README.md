...
### Starting download manually
If you prefer trigering download manually (ie on a click of a button) , rather than automatically when file(s) are selected, you can do so by overriding the default `onChange`event handler.
...
startUploadManually() {
	...
	files.forEach(file => this.fileUploader.startUpload(file));
}
